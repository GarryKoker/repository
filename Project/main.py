...
def krestiki_noliki(message):
    keyboard = telebot.util.quick_markup({
        ...
        "Крестики": {"callback_data": "Крестики"},
        "Нолики": {"callback_data": "Нолики"}

    })
    bot.send_message(message.chat.id, "Кем вы будете ходить? Крестики/Нолики", reply_markup = keyboard)

@bot.message_handler(func = lambda callback: callback.data == "Крестики")
def game_krestiki(callback: telebot.types.CallbackQuery):
    bot.send_message(callback.from_user.id, "Отлично! Вы играете крестиками")
    ...
    bot.send_message(callback.from_user.id, ".",reply_markup=gamekeyboard)


    @bot.message_handler(func = lambda callback: callback.data == "Нолики")
    def game_noliki(callback: telebot.types.CallbackQuery):
        bot.send_message(callback.from_user.id, "Отлично! Вы играете ноликами.")
